...
function check_new_backups() {
 echo -e "\n\n\033[36m[ cPTech Backup Report v2.2 ]\033[0m";
...
if [ "$legacy_enabled" == "yes" ]; then
 oldexcluded=$(egrep "LEGACY_BACKUP=0" /var/cpanel/users/* | wc -l 2>/dev/null);
 skip_file_ct=$(wc -l /etc/cpbackup-userskip.conf 2>/dev/null)
 if [ "$oldexcluded" -gt 0 -o "$skip_file_ct" ]; then
  echo -e "Legacy Backups:";
 fi
 if [ "$oldexcluded" -gt 0 ]; then echo -e "Number of real Legacy backup users disabled: \033[1;31m$oldexcluded\033[0m\n"; fi;
...
# TODO: math
newsuspended=$(egrep "=1" /var/cpanel/users/* 2>/dev/null | grep "SUSPENDED" | wc -l);
if [ "$newsuspended" != 0 ]; then
    echo -e "Users suspended: \033[1;31m$newsuspended\033[0m";
fi

if [ "$new_enabled" == "yes" ]; then
 newdis=$(egrep "^BACKUP=0" /var/cpanel/users/* 2>/dev/null | wc -l);
 echo -e "New Backup users disabled: \033[1;31m$newdis\033[0m";
 newen=$(egrep "^BACKUP=1" /var/cpanel/users/* | wc -l);
...
function show_recent_errors() {
    ...
    for i in `\ls $backlogdir`; do 
        ...
        \egrep -i "failed|error|load to go down|Unable|called at" $backlogdir/$i | cut -c -180 | sort | uniq -c ;
    done | tail;
