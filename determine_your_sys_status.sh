...
if [ "$keyu_pckg_chg_test" ]; then
    echo -e "\n"$red"Cmd1: keyutils-libs check failed. The rpm shows the following file changes: "$clroff
...
if [ "$cmd_2_chk" ]; then
    echo -e "\n"$red"Cmd2: Known bad package check failed. The following file is linked to libkeyutils.so.1: "$clroff
    echo $cmd_2_chk
    num_fails=$((num_fails+1))
fi
...
if [ "$cmd_3_chk" ]; then
    echo -e "\n"$red"Cmd3: libkeyutils libraries contain networking tools: "$clroff
    echo $cmd_3_chk
    num_fails=$((num_fails+1))
fi
...
if [ "$check_ipcs_lk" ];
 then echo -e "\n"$red"Cmd4: IPCS Check failed.  Doesn't necessarily mean anything:\n"$clroff$check_ipcs_lk;
 num_fails=$((num_fails+1))
fi
