...
for j in $doms; do
   ...
   for i in `cat $temp_dir/cptemp_eximbp | grep -B1 $dom | awk '{print $3}'`; do exim -Mvh $i | grep Subject; done | sort | uniq -c | sort -n | tail; done | awk '{split($4,encdata,"?"); command = (" base64 -d;echo"); if ($0~/UTF-8\?B/) {print "      "$1,$2,$3; print encdata[4] | command; close(command);} else {print}} END {print}'
