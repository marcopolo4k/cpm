...
echo; for j in $doms; do dom=$j; echo;echo "Count / Subjects for domain = $j:"; for i in `cat $temp_dir/cptemp_eximbp | grep -B1 $dom | awk '{print $3}'`; do exim -Mvh $i | grep Subject; done | sort | uniq -c | sort -n | tail; done | awk '{split($4,encdata,"?"); if ($0~/UTF-8\?B/) print encdata[4] | " base64 -d ; echo;echo"; print}'
